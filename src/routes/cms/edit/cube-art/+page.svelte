<script>
    ...
    function addService(section) {
        portfolioPage[section].services = [
            ...portfolioPage[section].services,
            {
                type: "",
                title: "",
    ...
    function removeService(section, index) {
        portfolioPage[section].services = portfolioPage[
            section
    ...
    function removeBulletPoint(section, serviceIndex, bulletIndex) {
        portfolioPage[section].services[serviceIndex].bulletPoints =
            portfolioPage[section].services[serviceIndex].bulletPoints.filter(
                (_, i) => i !== bulletIndex,
            );
    }
</script>
    ...
    <div class="edit-wrapper">
        <div class="wrapper">
            <div class="portfolio-cms">
                ...
                <div class="section">
                    <h3>Landing Text</h3>
                    {#each Object.entries(portfolioPage.cubeArt.landingText) as [key, value]}
                        <div>
                            <label for={`lego-${key}`}>{key}</label>
                            <input
                                id={`lego-${key}`}
                                bind:value={portfolioPage.cubeArt.landingText[
                                    key
                                ]}
                                type="text"
                            />
                    ...
                    <h3>Video Source</h3>
                    <input
                        bind:value={portfolioPage.cubeArt.videoSrc}
                        type="text"
                    />

                    <h3>How It Works</h3>
                    <textarea
                        bind:value={portfolioPage.cubeArt.howItWorks}
                        rows="4"
                        cols="50"
                    ></textarea>

                    <h3>FAQs</h3>
                    {#each portfolioPage.cubeArt.faqs as faq, index}
                        <div class="faq-item">
                    ...
                    <h3>Work Info</h3>
                    <input
                        bind:value={portfolioPage.cubeArt.workInfoTitle}
                        placeholder="Title"
                    />
                    <textarea
                        bind:value={portfolioPage.cubeArt.workInfoDescription}
                    ...
                    <input
                        bind:value={portfolioPage.cubeArt.gridImageSrc}
                        type="text"
                    />

                    <h3>What I Do</h3>
                    <textarea
                        bind:value={portfolioPage.cubeArt.whatIDO}
                        rows="4"
                        cols="50"
                    ></textarea>

                    <h3>Services</h3>
                    {#each portfolioPage.cubeArt.services as service, serviceIndex}
                        <div class="service-item">
                            ...
                            {#each service.bulletPoints as point, bulletIndex}
                                <div>
                                    ...
                                    class="bullet-point-item"
                                        bind:value={service.bulletPoints[
                                            bulletIndex
                                        ]}
                            ...
                            <button
                                on:click={() =>
                                    addBulletPoint("cubeArt", serviceIndex)}
                                >Add Bullet Point</button
                            >
                            <button
                                on:click={() =>
                                    removeService("cubeArt", serviceIndex)}
                                >Remove Service</button
                            >
                        </div>
                    {/each}
                    <button on:click={() => addService("cubeArt")}
                        >Add Service</button
                    >

                    <h3>Our Story</h3>
                    <textarea
                        bind:value={portfolioPage.cubeArt.ourStory}
                        rows="4"
                        cols="50"
                    ></textarea>
                    ...
                    <input
                        bind:value={portfolioPage.cubeArt.ourStoryImage}
                        type="text"
                    />
                </div>
            </div>
