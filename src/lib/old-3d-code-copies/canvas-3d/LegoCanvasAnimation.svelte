    ...
    function main() {
        ...
        onMouseMove = (event) => {
            ...
            for (let intersect of intersects) {
                ...
                if (obj) {
                    obj.moveDistance =
                        Math.round(mouseSpeed * 1000) / 10 / obj.body.mass;
                    obj.mouseSpeed = mouseSpeed;
                    obj.mouseAngle = mouseAngle;

                    // Calculate the center intersection percentage
                    const boundingBox = new THREE.Box3().setFromObject(
                        obj.mesh,
                    );
                    ...
                    const maxDistance =
                        boundingBox.min.distanceTo(boundingBox.max) / 2;
                    const distanceToCenter =
                        intersectionPoint.distanceTo(center);

                    let centerIntersectionPercentage =
                        (1 - distanceToCenter / maxDistance) * 100;
                    // Clamp the percentage between 0 and 100
                    let finalPercentage = Math.max(
                        0,
                        Math.min(100, centerIntersectionPercentage),
                    );

                    obj.centerIntersectionPercentage = finalPercentage;
                    //
                    console.log(
                        "obj.centerIntersectionPercentage",
                        obj.centerIntersectionPercentage,
                    );
        ...
        function updateObjects() {
            ...
            if (isModelsLoaded && !isVelocityDampened) {
                ...
                delay(100).then(() => {
                    ...
                    console.log(
                        "updating isModelsLoaded && !isVelocityDampened",
                    );
