<script>
    ...
    function main() {
            ...
            loader.load(
                ...
                (fbx) => {
                    ...
                    const zConstraint = new CANNON.PointToPointConstraint(
                        ...
                        world.bodies[0],
                        new CANNON.Vec3(boxBody.position.x, boxBody.position.y, 0)
                    );
        ...
        onMouseMove = (event) => {
            ...
            objects.forEach(obj => obj.isInteracted = false);
            ...
            for (let intersect of intersects) {
                ...
                if (obj) {
                    ...
                    const localIntersectPoint = obj.mesh.worldToLocal(intersect.point.clone());
                    ...
                    const isNearCorner = (
                        Math.abs(Math.abs(localIntersectPoint.x) - size.x / 2) < cornerThreshold ||
                        Math.abs(Math.abs(localIntersectPoint.y) - size.y / 2) < cornerThreshold ||
                        Math.abs(Math.abs(localIntersectPoint.z) - size.z / 2) < cornerThreshold
                    );

                    if (isNearCorner) {
                        const torqueStrength = 0.5;
                        const torque = new CANNON.Vec3(
                            rand(-torqueStrength, torqueStrength),
                            rand(-torqueStrength, torqueStrength),
                            0  // No rotation around z-axis
                        );
                        obj.body.torque.vadd(torque, obj.body.torque);
                        obj.moveDistance = 10;
                    }
        ...
        function updateObjects() {
            ...
            for (let obj of objects) {
                if (obj.isInteracted && obj.moveDistance > 0) {
                    ...
                    obj.body.velocity.set(0, 0, 0);
                    obj.body.angularVelocity.z = 0;  // Stop rotation around z-axis
                }
            }
