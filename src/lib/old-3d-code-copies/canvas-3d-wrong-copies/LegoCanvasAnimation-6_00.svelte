        ...
        onMouseMove = (event) => {
            ...
            if (intersects.length > 0) {
                ...
                if (obj) {
                    ...
                    const localIntersectPoint = obj.mesh.worldToLocal(
                    ...
                    const cornerThreshold = 0.1; // Reduced threshold for more precise corner detection
                    const isNearCorner =
                        Math.abs(Math.abs(localIntersectPoint.x) - size.x / 2) <
                            cornerThreshold ||
                        Math.abs(Math.abs(localIntersectPoint.y) - size.y / 2) <
                            cornerThreshold ||
                        Math.abs(Math.abs(localIntersectPoint.z) - size.z / 2) <
                            cornerThreshold;

                    if (isNearCorner) {
                        const torqueStrength = 0.3; // Reduced torque strength
        ...
        function updateObjects() {
            for (let obj of objects) {
                if (obj.isInteracted) {
                    ...
                    // Apply damping to gradually reduce velocities
                    obj.body.velocity.scale(0.98);
                    obj.body.angularVelocity.scale(0.98);

                    // Apply velocity threshold to stop small movements
